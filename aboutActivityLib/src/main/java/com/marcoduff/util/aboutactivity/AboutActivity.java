...
/**
 * AboutActivity è una utility per tutti gli sviluppatori Android per la
 * gestione della finestra di about dei loro programmi.
 * 
 * @author Marco "Duff" Palermo - www.marcoduff.com
 * @version 1.3
 * 
 *          modificato da Amedeo Lanza - Consulanza Informatica - spostate le
 *          stringhe traducibili in res/values/about.xml
 * 
 */
public class AboutActivity extends PreferenceActivity {
	...
	public static interface OnStartupEulaListener {
		/**
		 * Richiamata quando l'utente interagisce con la dialog di accettazione
		 * dell'EULA.
		 * 
		 * @param isAccepted
		 *            Ha valore <code>true</code> se l'utente accetta l'EULA,
	...
	public static interface OnStartupChangeLogListener {
		/**
		 * Richiamata quando viene rilevato un cambio di versione mentre si
		 * visualizza la dialog di ChangeLog in avvio.
		 * 
		 * @param oldVersion
		 *            La vecchia versione installata.
		 * @param newVersion
		 *            La nuova versione.
		 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * Ti chiedo di lasciare inalterata l'intera nota di copyright compreso
		 * il link al sito android.marcoduff.com per rispettare il mio lavoro,
		 * grazie!
		 * 
		 * I request you retain the full copyright notice below including the
		 * link to android.marcoduff.com to gives respect to my work, thanks!
		 */
		...
		Preference preferenceAuthorSite = getPreferenceFromUrlExtra(EXTRA_AUTHOR_SITE, R.string.aboutactivity_authorsite,
				R.string.aboutactivity_authorsite_summary);
		Preference preferenceAuthorMail = getPreferenceFromMailExtra(EXTRA_AUTHOR_EMAIL, R.string.aboutactivity_authoremail,
				R.string.aboutactivity_authoremail_summary);
		Preference preferenceAuthorTwitter = getPreferenceFromTwitterExtra(EXTRA_AUTHOR_TWITTER,
				R.string.aboutactivity_authortwitter, R.string.aboutactivity_authortwitter_summary);
		Preference preferenceReadme = getPreferenceFromRaw(RAW_README, R.string.aboutactivity_readme,
				R.string.aboutactivity_readme_summary);
		Preference preferenceFaq = getPreferenceFromRaw(RAW_FAQ, R.string.aboutactivity_faq, R.string.aboutactivity_faq_summary);
		Preference preferenceChangeLog = getPreferenceFromRaw(RAW_CHANGELOG, R.string.aboutactivity_changelog,
				R.string.aboutactivity_changelog_summary);
		Preference preferenceLicense = getPreferenceFromRaw(RAW_EULA, R.string.aboutactivity_license,
				R.string.aboutactivity_license_summary);
		Preference preferencePrivacy = getPreferenceFromRaw(RAW_PRIVACY, R.string.aboutactivity_privacy,
				R.string.aboutactivity_privacy_summary);
		Preference preferenceTodo = getPreferenceFromRaw(RAW_TODO, R.string.aboutactivity_todo, R.string.aboutactivity_todo_summary);
		Preference preferenceDonate = getPreferenceFromUrlExtra(EXTRA_DONATE_URL, R.string.aboutactivity_donate,
				R.string.aboutactivity_donate_summary);
		Preference preferenceMarket = getPreferenceMarket(EXTRA_SHOW_IN_MARKET, R.string.aboutactivity_market,
				R.string.aboutactivity_market_summary);
		Preference preferenceAuthorMarket = getPreferenceAuthorMarket(EXTRA_AUTHOR_MARKET_NAME,
				R.string.aboutactivity_authormarket, R.string.aboutactivity_authormarket_summary);
		Preference preferenceReportBug = getPreferenceFromUrlExtra(EXTRA_REPORT_BUG_URL, R.string.aboutactivity_reportbug,
				R.string.aboutactivity_reportbug_summary);

		PreferenceScreen preferenceScreen = this.getPreferenceManager().createPreferenceScreen(this);
		addPreferenceCategory(preferenceScreen, R.string.aboutactivity_info_category, preferenceVersion, preferenceAuthorName,
				preferenceAuthorSite, preferenceAuthorMail, preferenceAuthorTwitter);
		addPreferenceCategory(preferenceScreen, R.string.aboutactivity_app_category, preferenceReadme, preferenceFaq,
				preferenceChangeLog, preferenceLicense, preferencePrivacy, preferenceTodo);
		addPreferenceCategory(preferenceScreen, R.string.aboutactivity_miscellaneous_category, preferenceDonate, preferenceMarket,
				preferenceAuthorMarket, preferenceReportBug);
	 ...
	 * @return Restituisce <code>true</code> se l'utente ha già accettato
	 *         l'EULA, <code>false</code> altrimenti.
	 */
	...
	/**
	 * Visualizza la dialog del ChangeLog se viene rilevato un cambio di
	 * verisone.
	 * 
	 * @param context
	 *            Contesto di riferiemento.
	 * @return Restituisce <code>true</code> se non vi è stato cambio di
	 *         versione, <code>false</code> altrimenti.
	 */
	 ...
	 *            Contesto di riferiemento.
	 * @param titleResId
	 *            Risorsa per il titolo della dialog.
	 * @param filename
	 *            Nome del file asset da visualizzare.
	 */
	 ...
	 *            Contesto di riferiemento.
	 * @param authorName
	 *            Nome dell'autore o null.
	 * @param authorSite
	 *            Url del sito dell'autore o null.
	 * @param authorMail
	 *            EMail dell'autore o null.
	 * @param twitter
	 *            Username twitter dell'autore o null.
	 * @param donateUrl
	 *            Url per effettuare donazioni o null.
	 * @param showInAndroidMarketUrl
	 *            Se visualizzare o meno l'applicazione sul market android.
	 * @param authorAndroidMarketName
	 *            Nome dell'autore registrato nel market android.
	 * @param reportBugUrl
	 *            Url per sottomettere i bug.
	 * @return L'intent da usare per richiamare l'activity.
	 */
	public static Intent getAboutActivityIntent(Context context, String authorName, String authorSite, String authorMail,
			String twitter, String donateUrl, boolean showInAndroidMarketUrl, String authorAndroidMarketName, String reportBugUrl) {
		Intent intent = new Intent(context, AboutActivity.class);
		if (authorName != null)
			intent.putExtra(AboutActivity.EXTRA_AUTHOR_NAME, authorName);
		if (authorSite != null)
			intent.putExtra(AboutActivity.EXTRA_AUTHOR_SITE, authorSite);
		if (authorMail != null)
			intent.putExtra(AboutActivity.EXTRA_AUTHOR_EMAIL, authorMail);
		if (twitter != null)
			intent.putExtra(AboutActivity.EXTRA_AUTHOR_TWITTER, twitter);
		if (donateUrl != null)
			intent.putExtra(AboutActivity.EXTRA_DONATE_URL, donateUrl);
		if (showInAndroidMarketUrl)
			intent.putExtra(AboutActivity.EXTRA_SHOW_IN_MARKET, "true");
		if (authorAndroidMarketName != null)
			intent.putExtra(AboutActivity.EXTRA_AUTHOR_MARKET_NAME, authorAndroidMarketName);
		if (reportBugUrl != null)
			intent.putExtra(AboutActivity.EXTRA_REPORT_BUG_URL, reportBugUrl);
		return intent;
	}
	...
	private Preference getPreferenceFromUrlExtra(String extra, int titleResId, int summaryResId) {
		...
		if (url != null) {
			Preference preference = new Preference(this);
			preference.setTitle(titleResId);
			preference.setSummary(String.format(getString(summaryResId), url));
	...
	private Preference getPreferenceMarket(String extra, int titleResId, int summaryResId) {
		...
		if (flag != null && flag.equalsIgnoreCase("TRUE")) {
			...
			preference.setIntent(new Intent(Intent.ACTION_VIEW, Uri.parse(String.format("market://details?id=%1$s",
					this.getPackageName()))));
			return preference;
		} else
			return null;
	...
	private Preference getPreferenceAuthorMarket(String extra, int titleResId, int summaryResId) {
		...
		if (authorMarket != null) {
			...
			preference.setIntent(new Intent(Intent.ACTION_VIEW,
					Uri.parse(String.format("market://search?q=pub:%1$s", authorMarket))));
			return preference;
		} else
			return null;
	}
	...
	private Preference getPreferenceFromTwitterExtra(String extra, int titleResId, int summaryResId) {
		String url = this.getIntent().getStringExtra(extra);
		if (url != null) {
			url = String.format("http://www.twitter.com/%1$s", url);
			Preference preference = new Preference(this);
			preference.setTitle(titleResId);
			preference.setSummary(String.format(getString(summaryResId), url));
			preference.setIntent(new Intent(Intent.ACTION_VIEW, Uri.parse(url)));
	...
	private Preference getPreferenceFromMailExtra(String extra, int titleResId, int summaryResId) {
		...
		if (email != null) {
			...
			intent.putExtra(Intent.EXTRA_SUBJECT,
					String.format(getString(R.string.aboutactivity_email_subject), appName, currentVersion));
			Preference preference = new Preference(this);
			preference.setTitle(titleResId);
			preference.setSummary(String.format(getString(summaryResId), email));
